...
## Disclaimer
`strapi_lift` was created to assist with the migration of a Contentful instance to Strapi. We successfully migrated around ~11GB of assets and ~2000 entries from Contentful to Strapi with this tool. However, it is not a one-size-fits-all solution. The tool **WILL** require adjustments to fit your specific use case, especially if you have custom content types or complex relationships. You need to know `ruby` and to understand a good portion of the inner workings of this importer in order to use it. This tool was created for a one-time migration, **after all we all hope to always ever use scripts like these once**.
